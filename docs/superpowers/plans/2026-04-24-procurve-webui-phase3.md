...
> **For agentic workers:** REQUIRED SUB-SKILL: `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans`. The design spec is at `docs/superpowers/specs/2026-04-24-procurve-webui-phase3-design.md`.
>
...
**Convention:** each task declares files to create/modify + test plan + commit message. Code specifics are minimal — subagents should follow existing patterns from `app/api/backups.py`, `app/api/identity.py`, and `frontend/src/features/backups/` for style.
...
**Files to create:**
- `docs/superpowers/plans/phase3-status.md`
...
**Placeholder scan:** no TBDs, but some per-tab detail is deferred to the subagent to figure out based on existing procurve_client model shapes. That's intentional — the plan stays compact by trusting the established code patterns from Phase 2.

**Execution recommendation:** subagent-driven. One implementer + one spec reviewer + one code-quality reviewer per task unit (same as Phase 2). Configuration's 5 sub-tasks each get their own triple.
